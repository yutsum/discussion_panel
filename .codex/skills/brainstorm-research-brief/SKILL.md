...
4. Produce a concise shared brief with:
   - verified facts
   - source links
   - date context
   - explicit inferences
   - key unknowns
5. Save the brief to `brainstorms/<topic-slug>/shared-brief.md` unless the user specifies another path.
6. If a brief already exists, append a dated `Update` section at the end instead of rewriting the whole file unless repair is explicitly requested.
7. Hand the brief to `brainstorm-setup` or `brainstorm-moderator` as common context for all participants.
