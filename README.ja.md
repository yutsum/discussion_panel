...
各ペルソナはそれぞれ独立したコンテキストを持つサブエージェントとして動作します。各参加者が見てよいのは、トランスクリプト、共有 brief（存在する場合）、自分の persona 指示、自分の scratchpad だけです。さらに同一ラウンド内では、モデレーターが参加者を逐次実行し、先に話した参加者の発言を「ラウンド途中の transcript」として後続参加者に渡します。ラウンド間では、spawn コストを下げるために通常は同じ参加者に同じサブエージェントを再利用しつつ、毎回あらためて明示コンテキストを渡します。ラウンド完了後に `brainstorms/<topic-slug>/transcript.md` へ追記し、ユーザーは返答・方向転換・無言での続行を選べます。
...
`analyst` 以外の参加者は `brainstorms/<topic-slug>/agent-notes/` 以下にプライベートなスクラッチパッドを持てます。これらはコンパクトな作業メモ用であり、トランスクリプトには含まれません。ある参加者が他の参加者の scratchpad を読むことはありません。
...
議論中にファクトチェックや情報更新が予想される場合は、参加者に `analyst` を含めてください。`analyst` は通常ラウンドでは発言せず、ブリーフ更新・検証依頼・資料取り込み・ある事実自体が現在の論点になっていて調査が必要な場面でのみ参加します。
...
重要: モデレーターが UI 上で提示した「次は X/Y/Z で比較する」といった進行提案は、デフォルトでは参加者に見えていない前提です。ユーザーが明示的に採用してトランスクリプトに残さない限り、`continue` だけでその比較軸が参加者の知識になることはありません。ただし、同一ラウンド内で先に話した参加者の発言は、ラウンド途中の transcript を通じて後続参加者に見えます。
